//
//  MyFormatTextField.swift
//  DailyBanking
//
//  Created by Zsolt Molnár on 2022. 05. 26..
//
...
// DISCLAIMER!
// Due to the lack of better alternatives, this class was moved here from `AnyFormatKitSwiftUI` to fix an internal issue.
...
public struct FormatTextField: UIViewRepresentable {
    ...
    private var onEditingBeganHandler: ((String?) -> Void)?
    private var onEditingEndHandler: ((String?) -> Void)?
    private var onTextChangeHandler: ((String?) -> Void)?
    private var onClearHandler: (() -> Void)?
    private var onReturnHandler: (() -> Void)?

    // MARK: - Dependencies

    private let formatter: (TextInputFormatter & AnyFormatKit.TextFormatter & TextUnformatter)

    // MARK: - Life cycle

    public init(unformattedText: Binding<String>,
                placeholder: String? = nil,
                formatter: (TextInputFormatter & AnyFormatKit.TextFormatter & TextUnformatter)
    ) {
    ...
    public func makeUIView(context: Context) -> UIViewType {
        ...
        uiView.setContentCompressionResistancePriority(.defaultLow, for: .horizontal) // Fix horizontal compression resistance
    ...
    public func onEditingBegan(perform action: ((String?) -> Void)?) -> Self {
        var view = self
        view.onEditingBeganHandler = action
        return view
    }

    public func onEditingEnd(perform action: ((String?) -> Void)?) -> Self {
        var view = self
        view.onEditingEndHandler = action
        return view
    }

    public func onTextChange(perform action: ((String?) -> Void)?) -> Self {
        var view = self
        view.onTextChangeHandler = action
        return view
    }

    public func onClear(perform action: (() -> Void)?) -> Self {
        var view = self
        view.onClearHandler = action
        return view
    }

    public func onReturn(perform action: (() -> Void)?) -> Self {
        var view = self
    ...
    public class Coordinator: NSObject, UITextFieldDelegate {
        ...
        public var onEditingBegan: ((String?) -> Void)?
        public var onEditingEnd: ((String?) -> Void)?
